        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.works.update", [$work->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('field_of_work') ? 'is-invalid' : '' }}" name="field_of_work_id" id="field_of_work_id" required>
                        @foreach($field_of_works as $id => $entry)
                            <option value="{{ $id }}" {{ (old('field_of_work_id') ? old('field_of_work_id') : $work->field_of_work->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('organization') ? 'is-invalid' : '' }}" name="organization_id" id="organization_id" required>
                        @foreach($organizations as $id => $entry)
                            <option value="{{ $id }}" {{ (old('organization_id') ? old('organization_id') : $work->organization->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('designation') ? 'is-invalid' : '' }}" name="designation_id" id="designation_id" required>
                        @foreach($designations as $id => $entry)
                            <option value="{{ $id }}" {{ (old('designation_id') ? old('designation_id') : $work->designation->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('joining_date') ? 'is-invalid' : '' }}" type="text" name="joining_date" id="joining_date" value="{{ old('joining_date', $work->joining_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('resign_date') ? 'is-invalid' : '' }}" type="text" name="resign_date" id="resign_date" value="{{ old('resign_date', $work->resign_date) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_current_job') ? 'is-invalid' : '' }}" name="is_current_job" id="is_current_job" required>
                        ...
                        @foreach(App\Models\Work::IS_CURRENT_JOB_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_current_job', $work->is_current_job) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('view_on_publicly') ? 'is-invalid' : '' }}" name="view_on_publicly" id="view_on_publicly" required>
                        ...
                        @foreach(App\Models\Work::VIEW_ON_PUBLICLY_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('view_on_publicly', $work->view_on_publicly) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
