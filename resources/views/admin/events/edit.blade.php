        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('upazila') ? 'is-invalid' : '' }}" name="upazila_id" id="upazila_id" required>
                        @foreach($upazilas as $id => $entry)
                            <option value="{{ $id }}" {{ (old('upazila_id') ? old('upazila_id') : $event->upazila->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('union') ? 'is-invalid' : '' }}" name="union_id" id="union_id">
                        @foreach($unions as $id => $entry)
                            <option value="{{ $id }}" {{ (old('union_id') ? old('union_id') : $event->union->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('union'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('summary') ? 'is-invalid' : '' }}" name="summary" id="summary" required>{{ old('summary', $event->summary) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('batches') ? 'is-invalid' : '' }}" name="batches[]" id="batches" multiple>
                        @foreach($batches as $id => $batch)
                            <option value="{{ $id }}" {{ (in_array($id, old('batches', [])) || $event->batches->contains($id)) ? 'selected' : '' }}>{{ $batch }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('schools') ? 'is-invalid' : '' }}" name="schools[]" id="schools" multiple>
                        @foreach($schools as $id => $school)
                            <option value="{{ $id }}" {{ (in_array($id, old('schools', [])) || $event->schools->contains($id)) ? 'selected' : '' }}>{{ $school }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('event_status') ? 'is-invalid' : '' }}" name="event_status" id="event_status">
                        ...
                        @foreach(App\Models\Event::EVENT_STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('event_status', $event->event_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
